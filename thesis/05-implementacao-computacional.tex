...
Primeiramente, realiza-se o cálculo de grandezas macroscópicas, compreendendo a reconstrução de $\rho$, $\mathbf{u}$, $T$ e $p$ a partir dos momentos de $f_i$ e $g_i$, onde $\mathbf{u}$ é o vetor velocidade e $p$ a pressão local.
Em seguida, executa-se a colisão e o cálculo de equilíbrio, aplicando o operador BGK e determinando as distribuições de equilíbrio $f_i^{eq}$ e $g_i^{eq}$.
Esta etapa é a mais custosa computacionalmente devido à solução iterativa utilizando o método de Newton-Raphson.
Por fim, efetua-se o \textit{streaming}, que consiste no transporte das distribuições entre nós vizinhos da malha utilizando interpolação por pesos de distância inversa (\textit{Inverse Distance Weighting}, IDW).
...
Além disso, utiliza-se a memória compartilhada (\textit{shared memory}) em etapas específicas para reduzir o tráfego com a memória global (\textit{VRAM}), embora a implementação atual priorize o uso de registradores due à baixa complexidade geométrica da malha.
