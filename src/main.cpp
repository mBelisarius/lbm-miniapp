...
//------------------------------------------------------------------------------
// Fluid properties for compressible flows (Sod shock tube example)
...
template <typename Scalar_>
class LbmClassD3Q19 : public LbmClassBase<Scalar_, 3, 19> {
public:
  using Base = LbmClassBase<Scalar_, 3, 19>;
  using Base::Dim, Base::Speeds;

  ~LbmClassD3Q19() override = default;
...
void output_density(const Tensor<Scalar_, LbmClassType::Dim()>& rho, const Vector<Index, LbmClassType::Dim()>& sizes, Index step) {
  std::ofstream file("density_" + std::to_string(step) + ".dat");
...
public:
  ...
  void Init() {
    auto initializer = [&](const Vector<Index, LbmClassType_::Dim()>& indices) {
      // Assign values by reference
      Scalar_& rhoRef = rho_(indices[0], indices[1], indices[2]);
      Scalar_& uxRef = u_(indices[0], indices[1], indices[2], 0);
      Scalar_& uyRef = u_(indices[0], indices[1], indices[2], 1);
      Scalar_& uzRef = u_(indices[0], indices[1], indices[2], 2);

      // Apply shock tube condition along x-axis
      if (indices[0] < kSizes_[0] / 2) {
      ...
      for (Index i = 0; i < kLbmClass_.Speeds(); ++i) {
        ...
        f_(indices[0], indices[1], indices[2], i) = feq;
      }
      ...
      for (Index i = 0; i < kLbmClass_.Speeds(); ++i) {
        ...
        g_(indices[0], indices[1], indices[2], i) = geq;
      }
    };
  ...
  void Collision(Scalar_ dt) {
    ...
    // For standard LBM, c_s^2 = 1/3, so tau = 3 * viscosity + 0.5.
    Scalar_ tau = 3.0 * kFluidProps_.viscosity + 0.5;
    Scalar_ tauBar = tau / dt + 0.5;
    Scalar_ omega = 1.0 / tauBar;

    auto collisionOperator = [&](const Vector<Index, LbmClassType_::Dim()>& indices) {
      // Assign values by reference
      Scalar_& rhoRef = rho_(indices[0], indices[1], indices[2]);
      Scalar_& uxRef = u_(indices[0], indices[1], indices[2], 0);
      Scalar_& uyRef = u_(indices[0], indices[1], indices[2], 1);
      ...
      for (Index i = 0; i < kLbmClass_.Speeds(); ++i) {
        ...
        Scalar_& fRef = f_(indices[0], indices[1], indices[2], i);
        fRef -= omega * (fRef - feq);
      }
    };
  ...
  void Streaming() {
    ...
    auto streamingOperator = [&](const Vector<Index, LbmClassType_::Dim()>& indices) {
      for (Index i = 0; i < kLbmClass_.Speeds(); ++i) {
        // TODO: generalize for DnQm
        ...
        if ((indices[0] == 0 && kLbmClass_.Velocities(i, 0) < 0) || (indices[0] == kSizes_[0] - 1 && kLbmClass_.Velocities(i, 0) > 0)) {
          fAux_(indices[0], indices[1], indices[2], i) = f_(indices[0], indices[1], indices[2], kLbmClass_.Opposite(i));
          continue;
        }
        ...
        fAux_(indices[0], indices[1], indices[2], i) = f_(xSrc, ySrc, zSrc, i);
      }
    };
  ...
  void ComputeMacroscopic() {
    auto macroscopicTranform = [&](const Vector<Index, LbmClassType_::Dim()>& indices) {
      // Assign values by reference
      Scalar_& uxRef = u_(indices[0], indices[1], indices[2], 0);
      Scalar_& uyRef = u_(indices[0], indices[1], indices[2], 1);
      Scalar_& uzRef = u_(indices[0], indices[1], indices[2], 2);
      ...
      for (Index i = 0; i < kLbmClass_.Speeds(); ++i) {
        // TODO: generalize for DnQm
        Scalar_ fi = f_(indices[0], indices[1], indices[2], i);
        rho += fi;
      ...
      rho_(indices[0], indices[1], indices[2]) = rho;
      ...
      Scalar_ energy = 0.0;
      for (Index i = 0; i < kLbmClass_.Speeds(); ++i) {
        energy += g_(indices[0], indices[1], indices[2], i);
      }
      ...
      p_(indices[0], indices[1], indices[2]) = pressure;
      s_(indices[0], indices[1], indices[2]) = entropy;
    };
...
int main() {
  // Domain parameters
  constexpr Index kNx = 100;
  constexpr Index kNy = 10;
  constexpr Index kNz = 10;
  constexpr Index kSteps = 100;
  const auto sizes = Vector<Index, 3> { kNx, kNy, kNz };

  // Fluid properties for the shock tube:
  FluidProperties<double> fluidProps { };
  ...
  // LBM method
  static LbmClassD3Q19<double> lbmClass;
  LbmTube lbmTube(lbmClass, sizes, fluidProps);

  lbmTube.Init();
  output_density<double, LbmClassD3Q19<double>>(lbmTube.Rho(), sizes, 0);
  ...
  for (Index i = 1; i < kSteps + 1; ++i) {
    ...
    if (i % 10 == 0 && i > 0) {
      ...
      output_density<double, LbmClassD3Q19<double>>(lbmTube.Rho(), sizes, i);
    }
  }
