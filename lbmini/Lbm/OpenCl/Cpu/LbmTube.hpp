#ifndef LBMINI_OPENCL_CPU_LBMTUBE_HPP_
#define LBMINI_OPENCL_CPU_LBMTUBE_HPP_
...
typedef cl_int (CL_API_CALL *pfn_clCommandNDRangeKernelKHR)(cl_command_buffer_khr_ptr, cl_command_queue, const cl_properties*, cl_kernel, cl_uint, const size_t*, const size_t*, const size_t*, cl_uint, const void*, void*, void*);
 ...
 *
 * **Runtime Compilation (JIT).** OpenCL kernels are stored as raw strings and 
 * compiled at runtime via `cl::Program`. This avoids shipping pre-compiled binaries 
 ...
 *
 * **Thread Utilization and Dispatch Overhead.** The OpenCL CPU backend natively exhibits
 * ~75% CPU thread utilization due to the host dispatch overhead of queueing multiple kernels 
 * (collision, streaming) per time step. Unlike OpenMP's persistent thread teams (`#pragma omp parallel` 
 * wrapping the entire loop), the OpenCL runtime experiences significant overhead and worker stalling
 * as it waits for the command queue. To mitigate this, migrating the time loop into the kernel 
 * or using OpenCL Command-Buffers/Event-Graphs is recommended.
 */
...
public:
  ...
  LbmTube(const FluidData<Scalar>& fluid, const MeshData<Scalar, kDim_>& mesh, const ControlData<Scalar>& control, const PerformanceData& performance)
      ...
      std::ifstream kFile("lbmini/Lbm/OpenCl/Cpu/kernels.cl");
      if (!kFile.is_open()) {
      ...
      timeLoopKernel_ = cl::Kernel(program_, "timeLoopKernel");
      ...
      syncCounter_ = cl::Buffer(context_, CL_MEM_READ_WRITE, sizeof(int));
      syncFlag_ = cl::Buffer(context_, CL_MEM_READ_WRITE, sizeof(int));
  }

  ~LbmTube() override {
      if (use_cb_ && pfn_release_) {
          if (cbEven_) pfn_release_(cbEven_);
          if (cbOdd_) pfn_release_(cbOdd_);
      }
  }
  ...
  void Init() override {
      ...
      if (pfn_create_ && pfn_ndrange_ && pfn_finalize_ && pfn_enqueue_ && pfn_release_) {
          ...
          if (err != CL_SUCCESS) std::cerr << "CB Create Even err: " << err << std::endl;
          
          cbOdd_ = pfn_create_(1, &q, props, &err);
          if (err != CL_SUCCESS) std::cerr << "CB Create Odd err: " << err << std::endl;
          
          size_t global_work_size[1];
          if constexpr (std::is_same_v<LatticeType, lbmini::opencl::cpu::LatticeD2Q9<Scalar>>) {
          ...
          if (err != CL_SUCCESS) std::cerr << "CB Finalize Even err: " << err << std::endl;
          ...
          if (err != CL_SUCCESS) std::cerr << "CB Finalize Odd err: " << err << std::endl;
      }
  }
  ...
  void Step(bool save) override {
      if (use_cb_) {
          ...
          if (err != CL_SUCCESS) std::cerr << "CB Enqueue err: " << err << std::endl;
...
protected:
  void computeMacroscopic() {
      ...
      cl_int err = queue_.enqueueNDRangeKernel(macroscopicKernel_, cl::NullRange, cl::NDRange(gws), cl::NullRange);
      if(err != CL_SUCCESS) std::cerr << "Macro err: " << err << std::endl;
  }
  ...
  void seedEquilibria() {
      ...
      cl_int err = queue_.enqueueNDRangeKernel(seedKernel_, cl::NullRange, cl::NDRange(gws), cl::NullRange);
      if(err != CL_SUCCESS) std::cerr << "Seed err: " << err << std::endl;
  }

  void collisionAndEquilibria(const cl::Buffer& fIn, const cl::Buffer& gIn, cl_command_buffer_khr_ptr cb, cl_command_queue q, const size_t* gws_ptr, const size_t* lws_ptr) {
      size_t gws = N_;
      ...
      if (cb) {
          size_t cbgws[1] = { gws };
          cl_int err = pfn_ndrange_(cb, q, nullptr, collisionKernel_(), 1, nullptr, cbgws, nullptr, 0, nullptr, nullptr, nullptr);
          if(err != CL_SUCCESS) std::cerr << "Coll CB err: " << err << std::endl;
      } else {
          cl_int err = queue_.enqueueNDRangeKernel(collisionKernel_, cl::NullRange, cl::NDRange(gws), cl::NullRange);
          if(err != CL_SUCCESS) std::cerr << "Coll err: " << err << std::endl;
      }
  }

  void streamAndMacroscopic(const cl::Buffer& fIn, const cl::Buffer& gIn, const cl::Buffer& fOut, const cl::Buffer& gOut, cl_command_buffer_khr_ptr cb, cl_command_queue q, const size_t* gws_ptr, const size_t* lws_ptr) {
      size_t gws = N_;
      ...
      if (cb) {
          size_t cbgws[1] = { gws };
          cl_int err = pfn_ndrange_(cb, q, nullptr, streamKernel_(), 1, nullptr, cbgws, nullptr, 0, nullptr, nullptr, nullptr);
          if(err != CL_SUCCESS) std::cerr << "Stream CB err: " << err << std::endl;
      } else {
          cl_int err = queue_.enqueueNDRangeKernel(streamKernel_, cl::NullRange, cl::NDRange(gws), cl::NullRange);
          if(err != CL_SUCCESS) std::cerr << "Stream err: " << err << std::endl;
      }
  }
