#ifndef LBMINI_OPENMP_GPU_LBMTUBE_HPP_
#define LBMINI_OPENMP_GPU_LBMTUBE_HPP_
...
// members of a template class). Declaring them via `#pragma omp declare target`
 ...
 * `lbmini::openmp::cpu::LbmTube`. All GPU-specific choices are in the
 * dispatch and storage layers only.
 *
 * ### GPU design decisions
 *
 * **True device memory via `omp_target_alloc` + `is_device_ptr`.** Every
 * per-step array lives in raw device memory allocated once in the ctor.
 * Kernels receive raw device pointers via `is_device_ptr(...)` — zero
 ...
 *
 * **Namespace-scope `#pragma omp declare target` D2Q9 tables.** nvc++ 26.1
 ...
 * `constexpr int` arrays with `#pragma omp declare target` — safe because
...
protected:
   ...
   *
   * Offloaded via `#pragma omp target teams distribute parallel for`.
...
private:
  ...
  // #pragma omp declare target'd (which ICEs on nvc++ 26.1 for templates).
...
LbmTube<Scalar, LatticeType>::LbmTube(
  ...
  if (!rhoDev_ || !pDev_ || !temDev_ || !uDev_ || !fDev_ || !gDev_ || !fauxDev_ || !gauxDev_ || !lastGxDev_) {
    std::cerr << "[openmp::gpu::LbmTube] omp_target_alloc failed on device " << dev_
...
void LbmTube<Scalar, LatticeType>::computeMacroscopic() {
  ...
  #pragma omp target teams distribute parallel for collapse(2) \
      num_teams((N + 63) / 64) thread_limit(64) \
      is_device_ptr(pF, pG, pRho, pP, pT, pU)
...
void LbmTube<Scalar, LatticeType>::seedEquilibria() {
  ...
  #pragma omp target teams distribute parallel for collapse(2) \
      num_teams((N + 63) / 64) thread_limit(64) \
      is_device_ptr(pF, pG, pRho, pT, pU)
  for (int ci = 0; ci < nx; ++ci) {
    for (int cj = 0; cj < ny; ++cj) {
      const int cell = ci * ny + cj;
...
void LbmTube<Scalar, LatticeType>::collisionAndEquilibria() {
  ...
  #pragma omp target teams distribute parallel for collapse(2) \
      num_teams((N + 63) / 64) thread_limit(64) \
      is_device_ptr(pF, pG, pLastGx, pRho, pT, pU)
  for (int ci = 0; ci < nx; ++ci) {
    for (int cj = 0; cj < ny; ++cj) {
      const int cell = ci * ny + cj;
      const Scalar rho = pRho[cell];
...
void LbmTube<Scalar, LatticeType>::streamAndMacroscopic() {
  ...
  #pragma omp target teams distribute parallel for collapse(2) \
      num_teams((N + 63) / 64) thread_limit(64) \
      is_device_ptr(pF, pG, pFaux, pGaux, pRho, pP, pT, pU)
  for (int ci = 0; ci < nx; ++ci) {
    for (int cj = 0; cj < ny; ++cj) {
      const int cell = ci * ny + cj;
...
#endif // LBMINI_OPENMP_GPU_LBMTUBE_HPP_
