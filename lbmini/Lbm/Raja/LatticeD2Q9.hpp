#ifndef LBMINI_RAJA_LATTICED2Q9_HPP_
#define LBMINI_RAJA_LATTICED2Q9_HPP_
...
/**
 * @brief Portable D2Q9 lattice descriptor for the RAJA LBM backend.
 *
 * Numerically identical to `lbmini::plain::LatticeD2Q9`; every accessor is
 * decorated with `RAJA_HOST_DEVICE` (`__host__ __device__` on CUDA/HIP builds,
 * empty otherwise) so the very same descriptor can be called from host code and
 * from inside every `RAJA::forall` body, whichever execution policy the solver
 ...
 * subscript of such a member from a device lambda odr-uses a host object, which
 ...
 *
 * **Stateless and trivially copyable.** No data members, no virtual dispatch at
 * call sites: the descriptor is only ever used through `LatticeType::` static
 * calls, so nothing of it is captured into the `[=]` closure that RAJA ships to
 * the device.
 */
...
public:
  ...
  RAJA_HOST_DEVICE static constexpr Index Velocity(const Index index, const Index dir) {
  ...
  RAJA_HOST_DEVICE static constexpr Index Opposite(const Index index) {
   ...
   */
  RAJA_HOST_DEVICE static constexpr Scalar Weights(const Index idc, const Scalar tem) {
   ...
   */
  RAJA_HOST_DEVICE static constexpr Scalar Cshift(const Index idc, const Index dir, const Scalar Ushift) {
    return static_cast<Scalar>(Velocity(idc, dir)) + Ushift;
  }
};
} // namespace lbmini::raja

#endif // LBMINI_RAJA_LATTICED2Q9_HPP_
